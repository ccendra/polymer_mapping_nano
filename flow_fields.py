...
def quick_flow_plot(intensity_matrix, step_size, min_peak_intensity, peak_width, bend_tolerance, line_spacing,
                    rotate=False, seed_density=1, curve_resolution=2, spacing_resolution=5, angle_spacing_degrees=10,
                    max_overlap_fraction=0.5,
                    preview_sparsity=20, format_codes=[0, 1, 2], brightness=1, contrast=5, gamma=2):
    """This function covers the full process of creating flow plots from a matrix of intensity(row, col, theta).
    ...
    prepped_intensity_matrix = prepare_intensity_matrix(intensity_matrix, rotate=rotate)

    # Determine peak positions using a matrix of intensity as a function of (row, col, theta)
    peak_matrix = determine_peaks(intensity_matrix, min_peak_intensity, peak_width)

    # Create line seeds at each peak
    line_seeds = seed_lines(peak_matrix, step_size, seed_density=seed_density)

    # Extend line seeds to create full lines
    propagated_lines = propagate_lines(line_seeds, peak_matrix, step_size, bend_tolerance,
                                       curve_resolution=curve_resolution, max_grid_length=100)
    ...
    # Thin out lines, reducing overlap between lines and creating a more homogeneous line density.  This prevents the
    # illusion of high density in regions with good alignment, and makes the image more readable.
    trimmed_lines = trim_lines(propagated_lines, intensity_matrix.shape, step_size,
                               line_spacing, spacing_resolution, angle_spacing_degrees,
                               max_overlap_fraction=max_overlap_fraction, min_length=5, verbose=False)
    ...
    # Add intensity data to lines
    line_data = prepare_line_data(trimmed_lines, intensity_matrix, step_size)
    angle_data = line_data[2, :, :]
    intensity_data = np.array(line_data[4, :, :])
    n_dims, max_length, n_lines = line_data.shape
    ...
    for i, format_code in enumerate(format_codes):
        if format_code == 0:
            # Constant color, linewidth, and alpha
            r, g, b = color_by_angle(np.zeros((max_length, n_lines)))
            linewidth = np.ones((max_length, n_lines)) * 1
            alpha = np.ones((max_length, n_lines))
        elif format_code == 1:
            # Color by angle, alpha by intensity
            alpha = scale_values(smooth(intensity_data, 9), contrast=contrast, gamma=gamma, brightness=brightness)
            linewidth = np.ones((max_length, n_lines)) * 2
            r, g, b = color_by_angle(angle_data)
        elif format_code == 2:
            # Change brightness, contrast, and gamma values
            brightness_2 = 1  # Note that brightness values above 1 may cause errors if associated with an RGBA value.
            contrast_2 = 7
            gamma_2 = 3
            alpha = scale_values(smooth(intensity_data, 9), contrast=contrast_2, gamma=gamma_2, brightness=brightness_2)
            linewidth = np.ones((max_length, n_lines)) * 2
            r, g, b = color_by_angle(angle_data)
    ...
    return propagated_image, trimmed_image, formatted_plots, line_data, trimmed_lines
...
def propagate_lines(line_seeds, peak_matrix, matrix_step_size, bend_tolerance, curve_resolution=2, max_grid_length=100):
...
def main():
    """This function runs when this file is executed (as opposed to getting imported by another program).  It is used
     for testing during development and may not execute without the proper files available."""

    # Add folder containing code to path
    sys.path.append('/home2/luke/firefly')
    print(sys.path)

    # Import File
    input_path = "/home2/luke/tem_data/2017-09-26/dm4"
    output_path = "/home2/luke/projects/pbttt/data/2017-09-26_TEM"

    file_number = 0

    filename = choose_file(input_path, file_number, '.dm4')
    dm_path = input_path + '/' + filename
    big_datasets_path = output_path + '/big_datasets'

    for folder in ['/big_datasets', '/figures', '/diffraction_images', '/test']:
        if not os.path.isdir(output_path + folder):
            os.mkdir(output_path + folder)

    # Load Data
    q_min, q_max, q_step = (0.5, 3.0, 0.02)
    a_min, a_max, a_step = (-90, 90, 2.0)
    n_angles = int((a_max - a_min) / a_step)
    q_intensity_matrix = load_q_intensity_matrix(big_datasets_path, filename, q_min, q_max, q_step, a_step)

    # Specify dimensions for this data set
    camera_length, step_size, n_grid, q_per_pixel = read_metadata_from_filename(filename)

    # Get 3D normalized and background-subtracted data from 4D data
    peak_start_q, peak_end_q = 1.7, 1.9
    peak_start = int((peak_start_q - q_min) / q_step) + 1
    peak_end = int((peak_end_q - q_min) / q_step) + 1
    print(peak_start, peak_end)
    q_pi_reduced_matrix = q_intensity_matrix - np.median(q_intensity_matrix, axis=2)[:, :, None]
    intensity_matrix = np.mean(q_pi_reduced_matrix[:, peak_start:peak_end, :], axis=1)
    norm_intensity_matrix = normalize_and_reshape_intensity_matrix(intensity_matrix, n_grid, n_angles)

    # Start shared work flow ------
    min_peak_intensity = 1.0
    peak_width = 40.0
    bend_tolerance = 20.0
    line_spacing = 2
    quick_flow_plot(norm_intensity_matrix, step_size, min_peak_intensity, peak_width, bend_tolerance, line_spacing)
